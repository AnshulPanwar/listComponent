...
@each $prop-abbr, $prop-value in $kmx-spacing-properties {
    @each $interval-abbr, $interval-value in $kmx-spacing-intervals {
        .#{$prop-abbr}-#{$interval-abbr} {
            #{$prop-value}: #{$interval-value} !important;
        }
        @each $side-abbr, $side-value in $kmx-spacing-sides {
...
@each $abbreviation, $property in $kmx-sizing-properties {
    @each $key, $value in $kmx-spacing-intervals {
        .#{$abbreviation}-#{$key} { #{$property}: $value !important; }
    }
}
