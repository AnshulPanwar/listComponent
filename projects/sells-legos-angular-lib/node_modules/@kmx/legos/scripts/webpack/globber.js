...
/**
 * @fileoverview Scans the filesystem for files whose path matches a glob pattern,
 * and creates a Webpack chunk object to
 * compile those files.
 *
...
class Globber {
    ...
    /**
     * Finds all files that match the given path glob pattern and returns a Webpack
     * entry object containing each file as a separate "chunk" (key/value pair).
     *
     * The "name" (key) of each chunk is the chunk's input file path (relative to the project root)
     * without a file extension, and the value is the absolute path to the input file.
     *
